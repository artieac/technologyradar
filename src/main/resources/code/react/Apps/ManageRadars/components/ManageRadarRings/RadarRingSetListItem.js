...
import { addSelectedListItemToState, setShowEdit, addSetListToState } from  '../../redux/SetManagementReducer';
import { RadarRingRepository } from '../../../../Repositories/RadarRingRepository';

class RadarRingSetListItem extends React.Component{
    constructor(props){
        super(props);
         this.state = {
        };
    ...
    handleShowEditClick(event){
        this.props.storeSelectedListItem(this.props.radarRingSet);
    ...
    handleOnDeleteClick() {
        ...
        {
            this.radarRingRepository.deleteRadarRingSet(this.props.currentUser.id, this.props.radarRingSet.id, this.handleDeleteResponse);
        }
    }
    ...
    render() {
        if(typeof this.props.radarRingSet !== 'undefined'){
            return (
                <div className={ this.props.rowNum % 2 > 0 ? "row alternatingRow" : "row"}>
                    <div className="col-md-6">{this.props.radarRingSet.name } </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(RadarRingSetListItem);