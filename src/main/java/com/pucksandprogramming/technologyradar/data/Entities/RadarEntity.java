...
// Public
        ...
        @org.hibernate.annotations.NamedNativeQuery(name = "public_PublicFindCurrentRadarItemsByRadarType", query = "select tai.id, tai.TechnologyAssessmentId, tai.TechnologyId, tai.Details, tai.RadarRingId, tai.ConfidenceFactor, tai.RadarCategoryId, tai.State from TechnologyAssessments ta,TechnologyAssessmentItems tai INNER JOIN (SELECT MAX(Id) as Id FROM TechnologyAssessmentItems GROUP BY TechnologyId) tai2 ON tai.Id = tai2.Id WHERE ta.RadarUserId = :radarUserId AND ta.IsPublished=true AND ta.RadarTypeId = :radarTypeId AND tai.TechnologyAssessmentId = ta.Id", resultClass = RadarItemEntity.class),


// History
        @org.hibernate.annotations.NamedNativeQuery(name = "history_FindAllByUserType", query = "select ta.Id, ta.Name as Name, ta.AssessmentDate as AssessmentDate, ta.RadarUserId as RadarUserId,  ta.RadarTypeId as RadarTypeId, ta.IsPublished as IsPublished ,ta.IsLocked as IsLocked FROM TechnologyAssessments ta WHERE ta.RadarUserId = :radarUserId AND ta.RadarTypeId = :radarTypeId", resultClass = RadarEntity.class),
        ...
        @org.hibernate.annotations.NamedNativeQuery(name = "history_PublicFindCurrentRadarItemsByRadarType", query = "select tai.id, tai.TechnologyAssessmentId, tai.TechnologyId, tai.Details, tai.RadarRingId, tai.ConfidenceFactor, tai.RadarCategoryId, tai.State from TechnologyAssessments ta,TechnologyAssessmentItems tai INNER JOIN (SELECT MAX(Id) as Id FROM TechnologyAssessmentItems GROUP BY TechnologyId) tai2 ON tai.Id = tai2.Id WHERE ta.RadarUserId = :radarUserId AND ta.RadarTypeId = :radarTypeId AND tai.TechnologyAssessmentId = ta.Id", resultClass = RadarItemEntity.class),
        ...
        @org.hibernate.annotations.NamedNativeQuery(name = "findAllMostRecentByUserAndIsPublished", query = "select ta.Id, ta.Name as Name, ta.AssessmentDate as AssessmentDate, ta.RadarUserId as RadarUserId,  ta.RadarTypeId as RadarTypeId, ta.IsPublished as IsPublished ,ta.IsLocked as IsLocked FROM TechnologyAssessments ta WHERE (ta.id, ta.RadarTypeId) IN (SELECT MAX(ta2.Id), ta2.RadarTypeId FROM TechnologyAssessments ta2 WHERE ta2.RadarUserId = :radarUserId AND ta2.IsPublished = :isPublished GROUP BY ta2.RadarTypeId)", resultClass = RadarEntity.class),
        ...
        @org.hibernate.annotations.NamedNativeQuery(name = "findMostRecentByUserTypeAndIsPublished", query = "select ta.Id, ta.Name as Name, ta.AssessmentDate as AssessmentDate, ta.RadarUserId as RadarUserId,  ta.RadarTypeId as RadarTypeId, ta.IsPublished as IsPublished ,ta.IsLocked as IsLocked FROM TechnologyAssessments ta WHERE ta.id = (SELECT MAX(ta2.Id) FROM TechnologyAssessments ta2 WHERE ta2.RadarUserId = :radarUserId AND ta2.RadarTypeId = :radarTypeId AND ta2.IsPublished = :isPublished)", resultClass = RadarEntity.class),
