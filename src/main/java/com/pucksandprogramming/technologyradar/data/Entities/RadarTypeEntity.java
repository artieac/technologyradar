...
// Common
        ...
        @org.hibernate.annotations.NamedNativeQuery(name = "findMostRecentByUserAndId", query = "SELECT * from RadarTypes rt1 WHERE rt1.RadarUserId = :radarUserId AND rt1.Id = :radarTypeId AND rt1.Version =  (SELECT max(rt2.Version) FROM RadarTypes rt2 WHERE rt2.id = rt1.id) and rt2.State=1", resultClass = RadarTypeEntity.class),
        @org.hibernate.annotations.NamedNativeQuery(name = "findHistoryByRadarUserIdAndId", query = "SELECT * from RadarTypes rt WHERE rt.RadarUserId = :radarUserId AND rt.Id = :radarTypeId AND rt.State=1 ORDER BY rt.Id, rt.Version", resultClass = RadarTypeEntity.class),
     }
)
